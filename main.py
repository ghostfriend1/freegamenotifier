...
GAMERPOWER_BASE = httpsgamerpower.comapi
EPIC_LOCALE = en-US
EPIC_COUNTRY = US
...
class FreeGameNotifier(commands.Bot)
    ...
    async def fetch_new_free_games(self) - List[Dict[str, Any]]
            ...
            for item in data
                ...
                if gid and not self.db.is_seen(gid)
                    game = {
                        ...
                        open_giveaway_url item.get(open_giveaway_url, httpsgamerpower.com),
                        gamerpower_url fhttpsgamerpower.comgiveaway{gid},
                        source GamerPower,
        ...
        epic_data = await self._fetch_with_retry(
            httpsstore-site-backend-static.ak.epicgames.comfreeGamesPromotions,
                ...
                if price.get(discountPrice) in (0, Free) and elem.get(promotions, {}).get(promotionalOffers)
                    ...
                    if gid and not self.db.is_seen(gid)
                        new_games.append({
                            ...
                            worth Free (Promo),
                            end_date None,
                            open_giveaway_url fhttpsstore.epicgames.comen-USp{elem.get('productSlug', '')},
                            gamerpower_url httpsgamerpower.com,
                            source Epic,
                            is_promo True
                        })
    ...
    def create_game_embed(self, game Dict[str, Any]) - tuple[discord.Embed, ClaimView]
        ...
        claim_url = game.get(open_giveaway_url, httpsgamerpower.com)
        gamerpower_url = game.get(gamerpower_url, httpsgamerpower.com)

        worth = game.get(worth, Free)
        promo_text = fWas {worth} → FREE for limited time! if game.get(is_promo) and worth != Free else FREE now!

        # Build a Google Trends URL using URL encoding to avoid invalid characters
        ...
        embed.add_field(
            ...
            value=f[🔍 Google Search]({trends_url})nPlatform {platform}nSource {game.get('source', 'GamerPower')},
            inline=False,
        )
    ...
    async def fgstatus(self, interaction discord.Interaction)
        ...
        await interaction.response.send_message(
            fFree Game Notifier v3 Statusn✅ Onlinen📊 Games tracked {count}n🕒 Last check {last}n🔄 Checking every {CHECK_INTERVAL_MINUTES} minn🛡️ Public read-only APIs – 100% ban-proof,
            ephemeral=True
        )

    @app_commands.command(name=currentfree, description=List current freepromos (top 5))
    async def current_free(self, interaction discord.Interaction)
        await interaction.response.defer()
        games = await self.fetch_new_free_games()
        if not games
            await interaction.followup.send(No active freepromos right now.)
            return
        for game in games[5]
            embed, view = self.create_game_embed(game)
