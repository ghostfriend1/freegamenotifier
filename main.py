...
class Database:
    ...
    def is_seen(self, giveaway_id: str) -> bool:
        """Check if a giveaway ID has been seen already."""
    ...
    def mark_seen(self, giveaway_id: str, title: str, platform: str):
        """Mark a game as seen in the database."""
        self.conn.execute(
    ...
    def get_seen_count(self) -> int:
        """Get the count of games seen so far."""
...
class ClaimView(discord.ui.View):
    def __init__(self, claim_url: str, trends_url: str, gamerpower_url: str):
        """
        A Discord UI View with buttons for a claim link, Google trends search, and the gamer power page.
        """
        super().__init__(timeout=3600)
        self.add_item(
            discord.ui.Button(
                label="🔗 Claim Now", url=claim_url, style=discord.ButtonStyle.success
            )
        )
        self.add_item(
            discord.ui.Button(
                label="📈 Google Trends", url=trends_url, style=discord.ButtonStyle.primary
            )
        )
        self.add_item(
            discord.ui.Button(
                label="📋 GamerPower Page", url=gamerpower_url, style=discord.ButtonStyle.secondary
            )
        )
...
class FreeGameNotifier(commands.Bot):
    def __init__(self):
        """Initialize the bot with default intents and a database."""
    ...
    async def setup_hook(self):
        """Set up the bot once it starts up."""
        ...
        # Notify the channel that the bot is online
        channel = self.get_channel(CHANNEL_ID)
        if channel:
            await channel.send(
                "🚀 **Free Game Notifier v3 is online and monitoring all major platforms!**"
            )

    async def close(self):
        """Clean up resources upon closing the bot."""
        if self.session:
    ...
    async def _fetch_with_retry(
        ...
        """Fetch API data with exponential backoff retry."""
    ...
    async def check_free_games(self):
        """Scheduled task that checks for new free games at regular intervals."""
        try:
            logger.info("🔍 v3 Checking for new free / promo games...")
            new_games = await self.fetch_new_free_games()
            # If new games are found, send notifications to the channel
            if new_games:
                channel = self.get_channel(CHANNEL_ID)
                if not channel:
                    logger.error("Channel %s not found", CHANNEL_ID)
                    return
                # Mention either the role or default to @here
                ...
                for game in new_games:
                    ...
                    self.db.mark_seen(
                        game["id"], game["title"], game.get("platform", "Unknown")
                    )
            ...
            if channel:
                await channel.send(
                    "⚠️ **Notifier encountered a transient error** (will retry automatically)."
                )

    async def fetch_new_free_games(self) -> List[Dict[str, Any]]:
        """Fetch new free games from GamerPower and Epic Games APIs."""
        new_games: List[Dict[str, Any]] = []
        # Fetch from GamerPower
        ...
        # Fetch from Epic Games promotions
        ...
        if epic_data:
            ...
            for elem in elements:
                ...
                if price.get("discountPrice") in ("0", "Free") and elem.get("promotions", {}).get(
                    "promotionalOffers"
                ):
        ...
        # Deduplicate by title, platform, and ID
        seen: set[str] = set()
    ...
    def create_game_embed(self, game: Dict[str, Any]) -> tuple[discord.Embed, ClaimView]:
        """Create an embed and a view with buttons for a game notification."""
        title = game["title"]
        ...
        promo_text = (
            f"**Was {worth} → FREE for limited time!**"
            if game.get("is_promo") and worth != "Free"
            else "FREE now!"
        )

        # Properly URL encode the search query for Google
        query = quote_plus(f"{title} {platform} player count hype downloads trends 2026")
        ...
        embed.add_field(
            name="🔗 Claim Link",
            value=f"[Open Giveaway]({claim_url})",
            inline=False,
        )
        embed.add_field(
        ...
        embed.set_footer(
            text="v3 • Public APIs only • Zero ban risk • Bot by DeveloperGrok"
        )
    ...
    async def fgstatus(self, interaction: discord.Interaction):
        """Respond with the bot's current status, including games tracked and last check."""
    ...
    async def current_free(self, interaction: discord.Interaction):
        """Respond with up to 5 currently free or promo games."""
